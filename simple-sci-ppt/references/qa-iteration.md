...
5. Compare the markdown plan against the exported PPT preview: title, conclusion, visual objects, citations, page number, language choice, conclusion slide, selected layout archetype, layout box table, and speaker-note image-source entries must match the plan.
...
- Every slide with an inserted image, crop, generated plot, screenshot, schematic, or placeholder has an `Image sources:` section in speaker notes.
...
- Deck language is Chinese by default unless the user explicitly specified English or another target language.
...
- Image/crop/placeholder speaker notes include the source, page/figure, asset path or `n/a`, insertion status, and failure reason if any.
...
| Image/crop/placeholder lacks source entry in notes | add an `Image sources:` block with source, page/figure, asset/status, and failure reason |
...
| Missing paper image because crop tool is unavailable | insert a placeholder frame naming the paper and figure/page; mark visual status as `pending crop`; record the original source and failure reason in speaker notes |
| Slide title repeats a topic but not a conclusion | rewrite title as a declarative claim |
| Deck defaults to English without explicit request | translate visible text and notes to Chinese, while preserving proper nouns, citations, formulas, and code |
...
- Speaker-note image-source findings:
- Root causes:
