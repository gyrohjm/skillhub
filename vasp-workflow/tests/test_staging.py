...
def test_stage_inputs_link(tmp_path: Path) -> None:
    ...
    (tmp_path / "electronic" / "scf" / "WAVECAR").write_text("WAVEFUNCTION", encoding="utf-8")
    ...
    wavecar = tmp_path / "electronic" / "band" / "WAVECAR"
    assert linked.is_symlink()
    assert wavecar.is_symlink()
    assert linked.read_text(encoding="utf-8") == "CHARGE-DENSITY"
    assert wavecar.read_text(encoding="utf-8") == "WAVEFUNCTION"
    manifest = json.loads((tmp_path / "electronic" / "band" / "staged_inputs.json").read_text(encoding="utf-8"))
    assert [record["link"] for record in manifest["records"]] == [True, True]
    assert "sha256" not in manifest["records"][0]
